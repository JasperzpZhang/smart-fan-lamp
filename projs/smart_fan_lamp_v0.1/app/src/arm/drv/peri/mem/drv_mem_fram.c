...
status_t
FramConfig(MemHandle_t xHandle, MemConfig_t* pxConfig) {
...
status_t
FramCtrl(MemHandle_t xHandle, MemCtrlOperation_t xCtrlOp, uint32_t ulExtraPara) {
    status_t xRet;
    MemCtrl_t* pxCtrl = MEM_GET_CTRL(xHandle);
    ASSERT(NULL != pxCtrl);
    switch (xCtrlOp) {
        case MEM_CTRL_OP_FRAM_WP_NONE: xRet = prvWriteProtect(pxCtrl, FRAM_WP_NONE); break;
        case MEM_CTRL_OP_FRAM_WP_QUARTER: xRet = prvWriteProtect(pxCtrl, FRAM_WP_UPPER_QUARTER); break;
        case MEM_CTRL_OP_FRAM_WP_HALF: xRet = prvWriteProtect(pxCtrl, FRAM_WP_UPPER_HALF); break;
        case MEM_CTRL_OP_FRAM_WP_WHOLE: xRet = prvWriteProtect(pxCtrl, FRAM_WP_UPPER_WHOLE); break;
...
status_t
FramRead(MemHandle_t xHandle, uint32_t ulAddr, uint32_t ulLength, OUT uint8_t* pucData) {
    MemCtrl_t* pxCtrl = MEM_GET_CTRL(xHandle);
    ASSERT(NULL != pxCtrl);
...
status_t
FramWrite(MemHandle_t xHandle, uint32_t ulAddr, uint32_t ulLength, IN uint8_t* pucData) {
    MemCtrl_t* pxCtrl = MEM_GET_CTRL(xHandle);
    ASSERT(NULL != pxCtrl);
    if ((ulAddr < pxCtrl->xConfig.ulStAddr) || ((ulAddr + ulLength) > pxCtrl->xConfig.ulEdAddr)) {
        return status_err;
...
uint8_t
FramReadStatus(MemHandle_t xHandle) {
    MemCtrl_t* pxCtrl = MEM_GET_CTRL(xHandle);
    ASSERT(NULL != pxCtrl);
...
status_t
FramShowStatus(uint8_t ucStatus) {
...
static uint8_t
prvReadByte(MemCtrl_t* pxCtrl, uint32_t ulAddr) {
    uint8_t ucData;
...
static status_t
prvWriteByte(MemCtrl_t* pxCtrl, uint32_t ulAddr, uint8_t ucData) {
#if (MEM_FRAM_ADDR_WIDTH > 24)
    uint8_t ucAddrT;
#endif /* (MEM_FRAM_ADDR_WIDTH > 24) */
#if (MEM_FRAM_ADDR_WIDTH > 16)
...
static status_t
prvReadBulk(MemCtrl_t* pxCtrl, uint32_t ulAddr, uint32_t ulLength, OUT uint8_t* pucData) {
    uint32_t n;
#if (MEM_FRAM_ADDR_WIDTH > 24)
    uint8_t ucAddrT;
#endif /* (MEM_FRAM_ADDR_WIDTH > 24) */
...
static status_t
prvWriteBulk(MemCtrl_t* pxCtrl, uint32_t ulAddr, uint32_t ulLength, IN uint8_t* pucData) {
    uint32_t n;
#if (MEM_FRAM_ADDR_WIDTH > 24)
    uint8_t ucAddrT;
#endif /* (MEM_FRAM_ADDR_WIDTH > 24) */
...
static uint8_t
prvReadStatus(MemCtrl_t* pxCtrl) {
    uint8_t ucData;
...
static status_t
prvWriteStatus(MemCtrl_t* pxCtrl, uint8_t ucData) {
    FRAM_CS_L(pxCtrl);
    /* Enable write */
    prvSpiReadWriteByte(pxCtrl, REG_WREN);
    FRAM_CS_H(pxCtrl);
...
static status_t
prvWriteProtect(MemCtrl_t* pxCtrl, FramWriteProtect_t xWp) {
    Framstatus_t xStatus;
    xStatus.ucData = prvReadStatus(pxCtrl);
    switch (xWp) {
        case FRAM_WP_NONE: xStatus.xBit.BP = 0; break;
        case FRAM_WP_UPPER_QUARTER: xStatus.xBit.BP = 1; break;
        case FRAM_WP_UPPER_HALF: xStatus.xBit.BP = 2; break;
        case FRAM_WP_UPPER_WHOLE: xStatus.xBit.BP = 3; break;
        default:
            ASSERT(0); /* We should never get here! */
            break;
    }
...
static uint8_t
prvSpiReadWriteByte(MemCtrl_t* pxCtrl, uint8_t ucData) {
    uint8_t ucRecvd = 0;
