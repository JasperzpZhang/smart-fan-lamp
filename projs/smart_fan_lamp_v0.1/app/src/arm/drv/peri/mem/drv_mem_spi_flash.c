...
Spi_Flash_Init() {
    ...
    for (size_t i = 0; i < sizeof(flash_chip_table) / sizeof(Flash_Chip); i++) {
        if ((flash_chip_table[i].mf_id == flash.chip.mf_id) && (flash_chip_table[i].type_id == flash.chip.type_id)
...
Flash_Err
Spi_Flash_Config(SPI_HandleTypeDef* pSpi, GPIO_TypeDef* pCsPinPort, uint16_t nCsPin) {
    spi.spix = pSpi;
...
Flash_Err
Spi_Flash_Read_Status(uint8_t* status) {
...
Flash_Err
Spi_Flash_Write_Status(bool is_volatile, uint8_t status) {
    Flash_Err result;
    uint8_t cmd_data[2];
...
Flash_Err
Spi_Flash_Read(uint32_t addr, size_t size, uint8_t* data) {
    Flash_Err result;
...
/* Write flash data */
Flash_Err
Spi_Flash_Write(uint32_t addr, size_t size, const uint8_t* data) {
    Flash_Err result;
...
/* Erase flash*/
Flash_Err
Spi_Flash_Erase(uint32_t addr, size_t size) {
    Flash_Err result;
...
Flash_Err
Spi_Flash_Erase_Write(uint32_t addr, size_t size, const uint8_t* data) {
    Flash_Err result;
...
static Flash_Err
Set_Write_Enabled(bool enabled) {
    Flash_Err result;
...
static Flash_Err
Set_4_Byte_Address_Mode(bool enabled) {
    Flash_Err result;
    uint8_t cmd;
...
static status_t
Make_Adress_Byte_Array(uint32_t addr, uint8_t* array) {
    uint8_t len, i;
...
static Flash_Err
Page256_or_1_Byte_Write(uint32_t addr, size_t size, uint16_t write_gran, const uint8_t* data) {
    Flash_Err result;
...
static Flash_Err
Aai_Write(uint32_t addr, size_t size, const uint8_t* data) {
    Flash_Err result;
...
/* 读SFDP数据 */
static Flash_Err
read_sfdp_data(uint32_t addr, uint8_t* read_buf, size_t size) {
    /* 读SFDP命令为0x5A */
...
/* 读JEDEC基本参数头部，并验证是否可用 */
static bool
read_basic_header(sfdp_para_header* basic_header) {
...
/* 读JEDEC Flash参数表 */
static bool
read_basic_table(sfdp_para_header* basic_header) {
    ...
    switch (table[0] & 0x03) {
        /* 支持4K擦除 */
        case 1: flash.sfdp.erase_4k = true; break;

        /* 不支持4K擦除 */
        case 3: flash.sfdp.erase_4k = false; break;

        default: TRACE("Error: Uniform 4 KB erase supported information.\r\n"); return false;
    }

    /* 获取写粒度 */
    switch ((table[0] & (0x01 << 2)) >> 2) {
        /* 写粒度为1字节 */
        case 0: flash.sfdp.write_gran = 1; break;

        /* 写粒度为64字节或更大(一般为页编程256字节) */
        case 1: flash.sfdp.write_gran = 256; break;
    }

    /* 易失性状态寄存器块保护位 */
    switch ((table[0] & (0x01 << 3)) >> 3) {
        /* 状态寄存器是非易失性的 */
        case 0: flash.sfdp.sr_is_non_vola = true; break;
        ...
        case 1:
            ...
            switch ((table[0] & (0x01 << 4)) >> 4) {
                /* 写使能指令为0x50 */
                case 0: flash.sfdp.vola_sr_we_cmd = VOLATILE_SR_WRITE_ENABLE; break;

                /* 写使能指令为0x06 */
                case 1: flash.sfdp.vola_sr_we_cmd = CMD_WRITE_ENABLE; break;
            }
            break;
    }
    ...
    switch ((table[7] & (0x01 << 7)) >> 7) {
        case 0: flash.sfdp.capacity = 1 + (table2_temp >> 3); break;
...
/* 获取最合适的擦除方式 */
static size_t
flash_sfdp_get_suitable_eraser(uint32_t addr, size_t erase_size) {
...
static Flash_Err
Spi_Write_Read(uint8_t* write_buf, size_t write_size, uint8_t* read_buf, size_t read_size) {
    HAL_StatusTypeDef status;

    /* Determine the validity of parameters */
    if ((write_size != 0 && write_buf == NULL) || (read_size != 0 && read_buf == NULL)) {
        assert_param(0);
    }
