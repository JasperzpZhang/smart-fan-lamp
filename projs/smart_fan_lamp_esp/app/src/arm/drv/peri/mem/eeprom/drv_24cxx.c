...
uint8_t at24cxx_read_one_byte(uint16_t addr)
{
    ...
    {
        i2c_write_byte(0XA0);    /* 发送写命令, IIC规定最低位是0, 表示写入 */
        i2c_wait_ack();         /* 每次发送完一个字节,都要等待ACK */
        i2c_write_byte(addr >> 8);/* 发送高字节地址 */
    }
    else 
    {
        i2c_write_byte(0XA0 + ((addr >> 8) << 1));   /* 发送器件 0XA0 + 高位a8/a9/a10地址,写数据 */
    }
    
    i2c_wait_ack();             /* 每次发送完一个字节,都要等待ACK */
    i2c_write_byte(addr % 256);  /* 发送低位地址 */
    i2c_wait_ack();             /* 等待ACK, 此时地址发送完成了 */
    
    i2c_start();                /* 重新发送起始信号 */ 
    i2c_write_byte(0XA1);        /* 进入接收模式, IIC规定最低位是0, 表示读取 */
    i2c_wait_ack();             /* 每次发送完一个字节,都要等待ACK */
    i2c_read_byte(&temp, 0);    /* 接收一个字节数据 */
    i2c_stop();                 /* 产生一个停止条件 */
    return temp;
}
