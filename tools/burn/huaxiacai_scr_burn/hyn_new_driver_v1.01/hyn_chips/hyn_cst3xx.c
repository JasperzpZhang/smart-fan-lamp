...
static u32 cst3xx_read_checksum(void);
...
static int cst3xx_init(struct hyn_ts_data* ts_data)
{
    ...
    if(ret){
        HYN_INFO("cst3xx_enter_boot failed");
        return -1;
    }
    hyn_3xxdata->fw_updata_addr = (u8*)fw_bin;
    hyn_3xxdata->fw_updata_len = CST3xx_BIN_SIZE;
    hyn_3xxdata->hw_info.ic_fw_checksum = cst3xx_read_checksum();
    if(hyn_3xxdata->need_updata_fw ==0){
        hyn_wr_reg(hyn_3xxdata,0xA006EE,3,buf,0); //exit boot
        cst3xx_rst();
        mdelay(50);
        ...
        case NOMAL_MODE:
            ...
            if(buf[5]&0x80){ //key
                ...
                for(i = 0; i < finger_num; i++){
                    index = i*5;
                    hyn_3xxdata->rp_buf.pos_id[i] =  (buf[index]>>4)&0x0F;
                    hyn_3xxdata->rp_buf.event[i] =  (buf[index]&0x0F) == 0x06 ? 1 : 0;
                    hyn_3xxdata->rp_buf.pos_x[i] = ((u16)buf[index + 1]<<4) + ((buf[index + 3] >> 4) & 0x0F);
                    hyn_3xxdata->rp_buf.pos_y[i] = ((u16)buf[index + 2]<<4) + (buf[index + 3] & 0x0F);
                    hyn_3xxdata->rp_buf.pres_z[i] = buf[index + 4];
                    // HYN_INFO("report_id = %d, xy = %d,%d",hyn_3xxdata->rp_buf.pos_id[i],hyn_3xxdata->rp_buf.pos_x[i],hyn_3xxdata->rp_buf.pos_y[i]);
                }
            }
            break;
        case GESTURE_MODE:
...
static int cst3xx_updata_fw(u8 *bin_addr, u16 len)
{
    ...
    while(--retry){
        ret = cst3xx_enter_boot();
        if(ret){
            HYN_INFO("cst3xx_enter_boot faill");
            continue;
        }
        ...
        for (i=0; i<total_kbyte; i++) {
            ...
            ret = hyn_write_data(hyn_3xxdata, i2c_buf, 4);
#if HYN_TRANSFER_LIMIT
            ...
            ret |= hyn_write_data(hyn_3xxdata, i2c_buf, 1026);
#endif
    ...
    if(retry){
        hyn_wr_reg(hyn_3xxdata,0xA006EE,3,i2c_buf,0); //exit boot
        mdelay(2);
        cst3xx_rst();
        mdelay(50);
...
static int cst3xx_get_rawdata(u8 *buf, u16 len)
{
    return 0;
}

static int cst3xx_get_differ(u8 *buf, u16 len)
{
    return 0;
}
...
const struct hyn_ts_fuc cst3xx_fuc = {
    ...
    .tp_get_rawdata = cst3xx_get_rawdata,
    .tp_get_differ = cst3xx_get_differ,
